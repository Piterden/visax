...
visax.grid.Rates = function(config) {
    config = config || {};
    ...
    Ext.applyIf(config, {
        ...
        tbar: [{
            ...
            xtype: 'button',
            id: 'visax-rates-reload',
            text: 'reload',
            listeners: {
                'click': { fn: this.reloadRates, scope: this }
            }
        }]
    });
    visax.grid.Rates.superclass.constructor.call(this, config);
};
Ext.extend(visax.grid.Rates, MODx.grid.Grid, {
    reloadRates: function() {
        this.getStore().baseParams = {
            action: 'mgr/rate/getList',
            orphanSearch: 'modRate'
        };
    ...
    changeCategory: function(btn, e) {
        ...
        if (!this.changeCategoryWindow) {
            this.changeCategoryWindow = MODx.load({
                xtype: 'visax-rate-window-change-category',
                record: r,
                listeners: {
                    'success': {
                        fn: function(r) {
                            // this.refresh();
                            var sels = this.getSelectionModel().getSelections();
                            var cat = Ext.getCmp('visax-rate-category-combo').lastSelectionText;
                            var s = this.getStore();
    ...
    getBatchMenu: function() {
        ...
        bm.push({
            ...
            text: _('remove_rate') + '(s)',
            handler: this.rateRemove,
            scope: this
        });
        return bm;
    }
});
Ext.reg('visax-grid-rate', visax.grid.Rates);
...
visax.window.ChangeCategory = function(config) {
    ...
    Ext.applyIf(config, {
        ...
        url: visax.config.connectorUrl,
        baseParams: {
            action: 'mgr/rate/changecategory'
        },
        width: 400,
        fields: [{
            xtype: 'hidden',
...
Ext.reg('visax-rate-window-change-category', visax.window.ChangeCategory);
