...
visax.grid.Currencies = function(config) {
    config = config || {};
    ...
    Ext.applyIf(config, {
        ...
        tbar: [{
            ...
            xtype: 'button',
            id: 'visax-currencies-reload',
            text: 'reload',
            listeners: {
                'click': { fn: this.reloadCurrencies, scope: this }
            }
        }]
    });
    visax.grid.Currencies.superclass.constructor.call(this, config);
};
Ext.extend(visax.grid.Currencies, MODx.grid.Grid, {
    reloadCurrencies: function() {
        this.getStore().baseParams = {
            action: 'mgr/currency/getList',
            orphanSearch: 'modCurrency'
        };
    ...
    changeCategory: function(btn, e) {
        ...
        if (!this.changeCategoryWindow) {
            this.changeCategoryWindow = MODx.load({
                xtype: 'visax-currency-window-change-category',
                record: r,
                listeners: {
                    'success': {
                        fn: function(r) {
                            // this.refresh();
                            var sels = this.getSelectionModel().getSelections();
                            var cat = Ext.getCmp('visax-currency-category-combo').lastSelectionText;
                            var s = this.getStore();
    ...
    getBatchMenu: function() {
        ...
        bm.push({
            ...
            text: _('remove_currency') + '(s)',
            handler: this.currencyRemove,
            scope: this
        });
        return bm;
    }
});
Ext.reg('visax-grid-currency', visax.grid.Currencies);
...
visax.window.ChangeCategory = function(config) {
    ...
    Ext.applyIf(config, {
        ...
        url: visax.config.connectorUrl,
        baseParams: {
            action: 'mgr/currency/changecategory'
        },
        width: 400,
        fields: [{
            xtype: 'hidden',
            ...
            id: 'visax-currency-category-combo',
...
Ext.reg('visax-currency-window-change-category', visax.window.ChangeCategory);
